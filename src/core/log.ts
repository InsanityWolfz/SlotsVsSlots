...
const ABBR: Record<SymbolId, string> = { sword: 'SWD', shield: 'SHD', bolt: 'BLT', slime: 'SLM' };
...
export function formatRow(r: TurnRow): string {
  ...
  return (
    `T${r.turn} ${r.side === 'player' ? 'HERO ' : 'SLIME'} ${r.line.map((s) => ABBR[s]).join(' ')}${tier}${near}` +
...
export function rowsToCsv(rows: TurnRow[]): string {
  ...
  const esc = (v: unknown) => (Array.isArray(v) ? v.join('|') : String(v));
