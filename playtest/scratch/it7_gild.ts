...
const noneV: Record<string, number[]> = { taxed: [], frozen: [] };
for (const [gn, gift] of Object.entries(GIFTS)) {
  ...
  for (const mode of ['taxed', 'frozen']) {
    for (const cab of CABINET_ORDER) {
      ...
      snaps[cab].forEach((s, i) => {
        ...
        const hooks = mode === 'frozen' ? { preFight: (run: RunState, fight: Fight) => { if (fight.isMirror) fight.sides.enemy.hp = fight.sides.enemy.maxHp = Math.round(3 * machinePower(s) + 55 + 3 * ((run as any).p0 ? 0 : 0) + 3 * (machinePower(run) - machinePower(s)) * 0 + 3 * Math.max(0, machinePower(run) - machinePower(r0)) ); } } : undefined;
  ...
  console.log(`${gn.padEnd(20)} taxed ${avg(out.taxed).toFixed(1)} (${d('taxed') >= 0 ? '+' : ''}${d('taxed').toFixed(1)})  gift-untaxed ${avg(out.frozen).toFixed(1)} (${d('frozen') >= 0 ? '+' : ''}${d('frozen').toFixed(1)})   ${byCab.join(' ')}`);
}
