...
const VARIANTS: V[] = [
  { name: 'current' },
  ...[6, 7, 8, 9].map((k) => ({ name: 'HP = '+k+'x power+20, best-since, every3/2, no spec', hp: (r: RunState) => Math.round(k * power(r)) + 20, bestSince: true, every: 3, crackEvery: 2, noSpecial: true })),
  ...[7, 8].map((k) => ({ name: 'HP = '+k+'x power+20, best-since, every3/2, WITH spec', hp: (r: RunState) => Math.round(k * power(r)) + 20, bestSince: true, every: 3, crackEvery: 2 })),
  { name: 'HP = 8x power+20, best-since, every 4/3, no spec', hp: (r: RunState) => Math.round(8 * power(r)) + 20, bestSince: true, noSpecial: true },
];
