...
const FORMULAS: Record<string, F> = {
  ...
  'now+4/relic': (r) => Math.round(3 * machinePower(r) + 30 + 4 * r.player.relics.length),
  'now+4/act2relic': (r) => Math.round(3 * machinePower(r) + 55 + 4 * (r.player.relics.length - (r as any).r0)),
};
