...
for (const tierAll of [false, true]) for (const [vn, v] of Object.entries(VARIANTS)) { TIERALL = tierAll; if (vn.startsWith('now, cap') || vn.startsWith('typical 4c+55')) continue;
  ...
  for (const [pn, pol] of Object.entries(POLS)) {
    ...
    for (const cab of CABINET_ORDER) {
      ...
      snaps[cab].forEach((s, i) => {
        const hooks = { preFight: (run: RunState, f: Fight) => { if (TIERALL) { const t = new Set(run.player.gilded.filter((g) => g.tier).map((g) => g.enh)); run.player.gilded.forEach((g) => { if (t.has(g.enh)) g.tier = 2; }); } if (f.isMirror) { f.sides.enemy.hp = f.sides.enemy.maxHp = Math.round(v.hp(run)); f.sides.enemy.ability = { ...f.sides.enemy.ability!, power: v.cap(run) }; } } };
  ...
  console.log(`${tierAll ? 'TIER-ALL ' : 'tier-cell '}${vn.padEnd(24)} ${cells.join('  ')}  | commit ${byCab}`);
}
