...
// and what predicts a win.   npx tsx playtest/scratch/it7_mirrordeep.ts [N]
import { Fight } from '../../src/core/fight';
import { TUNE } from '../../src/core/enemies';
import { REFLECT_MIN } from '../../src/core/relics';
import { fullSets } from '../../src/core/run';
import { avg, batch, CABINET_ORDER, pct, type Policy, type RunRec } from './it7_lib';
...
const tele = { shown: 0, exact: 0, under: 0, over: 0, diffs: [] as number[], firstFireTurn: [] as number[], bankAtShow: [] as number[], spinsBeforeEnd: [] as number[] };
  ...
  preFight: (run: any, fight: Fight) => {
    ...
    (fight as any).step = () => {
      ...
      if (r.side === 'enemy' && !fight.over && ab && ab.every - e.charge <= 1) {
        pendingShown = Math.max(REFLECT_MIN, Math.min(ab.power, fight.reflectBank)); tele.bankAtShow.push(fight.reflectBank);
      }
      if (r.side === 'enemy') {
...
console.log(`Close wins (<25% HP left) ${pct(wins.filter(({ f }) => f.hpAfter / f.maxHp < 0.25).length, wins.length)}%  stomps (>75% HP left) ${pct(wins.filter(({ f }) => f.hpAfter / f.maxHp > 0.75).length, wins.length)}%`);
console.log(`Reflection fired >=1: ${pct(ms.filter(({ f }) => f.reflects.length >= 1).length, ms.length)}%  >=2: ${pct(ms.filter(({ f }) => f.reflects.length >= 2).length, ms.length)}%  >=3: ${pct(ms.filter(({ f }) => f.reflects.length >= 3).length, ms.length)}%`);
console.log(`AT LEAST honesty: hit < shown ${tele.over} times of ${tele.shown}`);
by('win by cabinet', (m) => m.r.cabinet);
by('win by mirror HP band', (m) => (m.f.enemyHp < 100 ? '<100' : m.f.enemyHp < 150 ? '100-149' : m.f.enemyHp < 250 ? '150-249' : '250+'));
by('reflection fired by mirror HP band', (m) => (m.f.enemyHp < 100 ? '<100' : m.f.enemyHp < 150 ? '100-149' : m.f.enemyHp < 250 ? '150-249' : '250+') + (m.f.reflects.length ? ':fired' : ':none'));
by('win by tier IIs owned', (m) => String(Math.min(3, m.r.gilds.length ? (m.r as any).tiers ?? 0 : 0)));
